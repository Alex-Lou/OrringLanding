...
export const translations: Record<LangCode, Record<string, string>> = {
  fr: {
    ...
    exp4Body: "⭕ Mon Cycle : ton tableau de bord — anneau animé avec countdown, état actuel, dates clés, action suivante (mettre/retirer) et mini-minuteur pour un retrait temporaire.\n\n📅 Calendrier : 12 mois en un coup d'œil avec codes couleur. Tape un jour pour ajouter des notes.\n\n🩸 Mes périodes : un onglet dédié au suivi de tes règles, indépendant de l'anneau.\n\n🕐 Historique : tous tes cycles passés, en cours et à venir.\n\n📖 Explications : contexte sur le cycle et la contraception (cette page).\n\n⚙️ Réglages : date de référence, notifications, mode sombre, langue, nom.",
    exp5Title: 'Mes périodes — un suivi dédié',
    exp5Body: "« Mes périodes » est un onglet à part entière, complètement séparé du calendrier de l'anneau : aucune donnée ne se mélange. Le parcours est guidé : tape le premier jour de tes règles, choisis l'intensité du flux (léger / normal / abondant), continue à taper chaque jour suivant, puis marque le dernier jour pour clore le cycle. Chaque journée garde sa propre intensité, et le calendrier affiche un véritable dégradé du léger à l'abondant. Dès le deuxième cycle enregistré, l'app calcule ta durée moyenne sur les 6 derniers cycles et te prédit la prochaine date de début. Trois petits rappels automatiques t'accompagnent autour de cette date : 2 jours avant, le jour J, et 3 jours après si rien n'a été noté. Ce suivi est totalement indépendant de l'anneau — il fonctionne aussi pour les personnes qui n'en utilisent pas.",
  ...
  en: {
    ...
    exp4Body: "⭕ My Cycle: your dashboard — animated ring with countdown, current status, key dates, next action (insert/remove) and a mini 3h timer for temporary removals.\n\n📅 Calendar: 12 months at a glance with color codes. Tap a day to add notes.\n\n🩸 My Periods: a dedicated tab for tracking your periods, independent of the ring.\n\n🕐 History: all your past, current and upcoming cycles.\n\n📖 Explanations: context on cycles and contraception (this page).\n\n⚙️ Settings: reference date, notifications, dark mode, language, name.",
    exp5Title: 'My Periods — a dedicated tracker',
    exp5Body: "\"My Periods\" is its own tab, completely separate from the ring's calendar — no data is ever mixed. The flow is guided: tap the first day of your period, choose the flow intensity (light / normal / heavy), keep tapping each following day, then mark the last day to close the cycle. Each day stores its own intensity, and the calendar displays a real gradient from light to heavy. From the second logged cycle onward, the app calculates your average length over the last 6 cycles and predicts the next start date. Three gentle automatic reminders surround that date: 2 days before, the day itself, and 3 days late if nothing has been logged. The tracker works fully independently from the ring — it's just as useful for people who don't use one.",
  ...
  nl: {
    ...
    exp4Body: "⭕ Mijn Cyclus: je dashboard — geanimeerde ring met aftelling, huidige status, sleuteldata, volgende actie (plaatsen/verwijderen) en een mini-timer voor tijdelijk verwijderen.\n\n📅 Kalender: 12 maanden in één oogopslag met kleurcodes. Tik op een dag om notities toe te voegen.\n\n🩸 Mijn Menstruatie: een apart tabblad om je menstruatie bij te houden, los van de ring.\n\n🕐 Geschiedenis: al je vroegere, lopende en komende cycli.\n\n📖 Uitleg: context over de cyclus en anticonceptie (deze pagina).\n\n⚙️ Instellingen: referentiedatum, meldingen, donkere modus, taal, naam.",
    exp5Title: 'Mijn Menstruatie — een apart overzicht',
    exp5Body: "\"Mijn Menstruatie\" is een volledig apart tabblad, helemaal losgekoppeld van de ringkalender — er worden nooit gegevens gemengd. Het verloopt stap voor stap: tik op de eerste dag van je menstruatie, kies de intensiteit (licht / normaal / hevig), blijf elke volgende dag aantikken en markeer de laatste dag om de cyclus af te sluiten. Elke dag bewaart zijn eigen intensiteit, en de kalender toont een echte verloopkleur van licht naar hevig. Vanaf je tweede geregistreerde cyclus berekent de app je gemiddelde duur op basis van de laatste 6 cycli en voorspelt de volgende startdatum. Drie zachte automatische herinneringen begeleiden die datum: 2 dagen ervoor, op de dag zelf en 3 dagen later als er nog niets is genoteerd. Dit overzicht werkt volledig onafhankelijk van de ring — ook handig voor mensen die geen ring gebruiken.",
    downloadsCount: 'downloads',
  ...
  ru: {
    ...
    exp4Body: '⭕ Мой цикл: твоя панель — анимированное кольцо с обратным отсчётом, текущий статус, ключевые даты, следующее действие (установить/снять) и мини-таймер на 3ч для временного снятия.\n\n📅 Календарь: 12 месяцев сразу с цветовыми кодами. Нажми на день, чтобы добавить заметку.\n\n🩸 Мои месячные: отдельная вкладка для отслеживания менструации, независимая от кольца.\n\n🕐 История: все прошлые, текущие и предстоящие циклы.\n\n📖 Пояснения: контекст о цикле и контрацепции (эта страница).\n\n⚙️ Настройки: дата начала, уведомления, тёмная тема, язык, имя.',
    exp5Title: 'Мои месячные — отдельный трекер',
    exp5Body: '«Мои месячные» — это самостоятельная вкладка, полностью отделённая от календаря кольца: данные никогда не смешиваются. Всё ведётся пошагово: отметь первый день месячных, выбери интенсивность (слабая / обычная / обильная), продолжай отмечать каждый следующий день и поставь отметку на последнем дне, чтобы закрыть цикл. Каждый день сохраняет свою интенсивность, а календарь показывает настоящий градиент от слабой до обильной. Со второго записанного цикла приложение рассчитывает среднюю длину по последним 6 циклам и предсказывает дату следующего начала. Около этой даты приходят три мягких автоматических напоминания: за 2 дня, в сам день и через 3 дня после, если ничего не отмечено. Трекер работает полностью независимо от кольца — он подойдёт и тем, кто не пользуется кольцом.',
  ...
  es: {
    ...
    exp4Body: "⭕ Mi Ciclo: tu panel — anillo animado con cuenta atrás, estado actual, fechas clave, próxima acción (poner/retirar) y un mini temporizador para retiradas temporales.\n\n📅 Calendario: 12 meses de un vistazo con códigos de colores. Toca un día para añadir notas.\n\n🩸 Mis Reglas: una pestaña dedicada al seguimiento de tus reglas, independiente del anillo.\n\n🕐 Historial: todos tus ciclos pasados, en curso y futuros.\n\n📖 Explicaciones: contexto sobre el ciclo y la anticoncepción (esta página).\n\n⚙️ Ajustes: fecha de referencia, notificaciones, modo oscuro, idioma, nombre.",
    exp5Title: 'Mis Reglas — un seguimiento dedicado',
    exp5Body: "«Mis Reglas» es una pestaña independiente, totalmente separada del calendario del anillo: los datos nunca se mezclan. El recorrido es guiado: toca el primer día de tu regla, elige la intensidad del flujo (ligero / normal / abundante), sigue tocando cada día siguiente y marca el último día para cerrar el ciclo. Cada día guarda su propia intensidad y el calendario muestra un degradado real de ligero a abundante. Desde el segundo ciclo registrado, la app calcula tu duración media sobre los últimos 6 ciclos y predice la próxima fecha de inicio. Tres recordatorios automáticos suaves acompañan esa fecha: 2 días antes, el mismo día y 3 días después si no has anotado nada. Este seguimiento funciona de forma totalmente independiente del anillo — también es útil para quien no usa anillo.",
  ...
  pt: {
    ...
    exp4Body: "⭕ Meu Ciclo: seu painel — anel animado com contagem regressiva, estado atual, datas-chave, próxima ação (colocar/retirar) e mini-timer para remoção temporária.\n\n📅 Calendário: 12 meses de uma vez com códigos de cores. Toque em um dia para adicionar notas.\n\n🩸 Minha Menstruação: uma aba dedicada ao acompanhamento da menstruação, independente do anel.\n\n🕐 Histórico: todos os seus ciclos passados, em curso e futuros.\n\n📖 Explicações: contexto sobre o ciclo e a contracepção (esta página).\n\n⚙️ Configurações: data de referência, notificações, modo escuro, idioma, nome.",
    exp5Title: 'Minha Menstruação — um acompanhamento dedicado',
    exp5Body: "\"Minha Menstruação\" é uma aba à parte, completamente separada do calendário do anel — os dados nunca se misturam. O fluxo é guiado: toque no primeiro dia da menstruação, escolha a intensidade (leve / normal / intenso), continue tocando em cada dia seguinte e marque o último dia para fechar o ciclo. Cada dia guarda sua própria intensidade e o calendário exibe um gradiente real, do leve ao intenso. A partir do segundo ciclo registrado, o app calcula sua duração média com base nos 6 últimos ciclos e prevê a próxima data de início. Três lembretes automáticos suaves acompanham essa data: 2 dias antes, no próprio dia e 3 dias depois se nada tiver sido registrado. O acompanhamento funciona totalmente independente do anel — também serve para quem não usa anel.",
    downloadsCount: 'downloads',
  ...
  de: {
    ...
    exp4Body: "⭕ Mein Zyklus: dein Dashboard — animierter Ring mit Countdown, aktueller Status, wichtige Daten, nächste Aktion (einsetzen/entfernen) und Mini-Timer für vorübergehendes Entfernen.\n\n📅 Kalender: 12 Monate auf einen Blick mit Farbcodes. Tippe auf einen Tag, um Notizen hinzuzufügen.\n\n🩸 Meine Periode: ein eigener Tab zum Tracken deiner Periode, unabhängig vom Ring.\n\n🕐 Verlauf: alle vergangenen, aktuellen und kommenden Zyklen.\n\n📖 Erklärungen: Kontext zu Zyklus und Verhütung (diese Seite).\n\n⚙️ Einstellungen: Referenzdatum, Benachrichtigungen, dunkler Modus, Sprache, Name.",
    exp5Title: 'Meine Periode — ein eigener Tracker',
    exp5Body: "„Meine Periode\" ist ein eigenständiger Tab, vollständig getrennt vom Ring-Kalender — Daten werden niemals vermischt. Der Ablauf ist geführt: Tippe auf den ersten Tag deiner Periode, wähle die Stärke (leicht / normal / stark), tippe an jedem weiteren Tag weiter und markiere den letzten Tag, um den Zyklus abzuschließen. Jeder Tag speichert seine eigene Stärke, und der Kalender zeigt einen echten Verlauf von leicht bis stark. Ab dem zweiten erfassten Zyklus berechnet die App deine durchschnittliche Länge auf Basis der letzten 6 Zyklen und sagt das nächste Startdatum voraus. Drei sanfte automatische Erinnerungen begleiten dieses Datum: 2 Tage vorher, am Tag selbst und 3 Tage später, wenn nichts eingetragen wurde. Der Tracker funktioniert völlig unabhängig vom Ring — er ist genauso nützlich für Personen, die keinen Ring verwenden.",
    downloadsCount: 'Downloads',
  ...
  ar: {
    ...
    exp4Body: "⭕ دورتي: لوحة التحكم الخاصة بك — حلقة متحركة مع عد تنازلي، الحالة الراهنة، التواريخ المهمة، الإجراء التالي (إدخال/إزالة) ومؤقت صغير للإزالة المؤقتة.\n\n📅 التقويم: 12 شهراً بنظرة واحدة مع رموز ألوان. انقري على يوم لإضافة ملاحظات.\n\n🩸 دورتي الشهرية: تبويب مخصّص لمتابعة الدورة الشهرية، مستقل تماماً عن الحلقة.\n\n🕐 السجل: جميع دوراتك السابقة والحالية والقادمة.\n\n📖 الشروحات: سياق حول الدورة ووسائل منع الحمل (هذه الصفحة).\n\n⚙️ الإعدادات: التاريخ المرجعي، الإشعارات، الوضع الداكن، اللغة، الاسم.",
    exp5Title: 'دورتي الشهرية — متابعة مخصّصة',
    exp5Body: "«دورتي الشهرية» هو تبويب مستقل بذاته، منفصل تماماً عن تقويم الحلقة، ولا تختلط البيانات أبداً. المسار موجَّه خطوة بخطوة: انقري على اليوم الأول من دورتك، اختاري شدّة التدفق (خفيف / عادي / غزير)، تابعي النقر على كل يوم تالٍ، ثم حدّدي اليوم الأخير لإغلاق الدورة. يحتفظ كل يوم بشدّته الخاصة، ويعرض التقويم تدرّجاً حقيقياً من الخفيف إلى الغزير. ابتداءً من الدورة الثانية المسجَّلة، يحسب التطبيق متوسط طول دورتك بناءً على آخر 6 دورات ويتنبّأ بتاريخ بدء الدورة التالية. ترافقك ثلاثة تذكيرات تلقائية لطيفة حول هذا التاريخ: قبل يومين، في اليوم نفسه، وبعد 3 أيام من التأخر إذا لم يُسجَّل شيء. تعمل هذه الميزة باستقلال تام عن الحلقة — وهي مفيدة أيضاً لمن لا يستخدمن الحلقة.",
  ...
  zh: {
    ...
    exp4Body: "⭕ 我的周期：你的仪表板——带倒计时的动画环、当前状态、关键日期、下一步操作（放入/取出），以及临时取出的迷你计时器。\n\n📅 日历：12个月一目了然，带颜色编码。点击某一天添加笔记。\n\n🩸 我的经期：专门用于追踪经期的标签页，独立于避孕环。\n\n🕐 历史：所有过去的、进行中的和即将到来的周期。\n\n📖 说明：关于周期和避孕的背景（本页面）。\n\n⚙️ 设置：参考日期、通知、深色模式、语言、姓名。",
    exp5Title: '我的经期——专属追踪',
    exp5Body: "「我的经期」是一个独立的标签页，与避孕环的日历完全分开，数据互不混杂。流程很简单：点击经期的第一天，选择经量（少量／正常／量多），之后每一天继续点击，最后一天再点一下以结束本次周期。每一天都会保存各自的经量，日历会以真实的渐变显示从少量到量多的变化。从第二个记录的周期起，应用会根据最近 6 个周期计算平均长度，并预测下次的开始日期。围绕该日期会有三个温柔的自动提醒：提前 2 天、当天，以及如果还没有记录则在延迟 3 天后提醒。该追踪完全独立于避孕环——也适合不使用避孕环的用户。",
    downloadsCount: '次下载',
    feedbackTitle: '给我们留言',
  ...
  ja: {
    ...
    exp4Body: "⭕ マイサイクル：あなたのダッシュボード——カウントダウン付きのアニメーションリング、現在の状態、重要な日付、次のアクション（装着/取り外し）、一時的な取り外し用のミニタイマー。\n\n📅 カレンダー：12ヶ月を一目で、カラーコード付き。日をタップしてメモを追加。\n\n🩸 マイ生理：生理を記録するための専用タブ。リングとは独立しています。\n\n🕐 履歴：過去、進行中、今後のすべてのサイクル。\n\n📖 説明：周期と避妊についての背景（このページ）。\n\n⚙️ 設定：基準日、通知、ダークモード、言語、名前。",
    exp5Title: 'マイ生理 — 専用の記録タブ',
    exp5Body: "「マイ生理」はリングのカレンダーから完全に独立した専用タブで、データが混ざることはありません。流れはシンプルです：生理の初日をタップし、量（少なめ／普通／多め）を選び、翌日以降も毎日タップ、最終日をマークしてサイクルを締めくくります。各日にそれぞれの量が保存され、カレンダーには少なめから多めへの本物のグラデーションが表示されます。2回目に記録したサイクルから、アプリは直近6サイクルの平均日数を計算し、次回の開始日を予測します。その予測日に合わせて、優しい自動リマインダーが3回届きます：2日前、当日、そして何も記録がないまま3日遅れた場合。このトラッカーはリングから完全に独立して動作し、リングを使っていない方にも便利です。",
